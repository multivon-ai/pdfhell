...
- ✅ **What survives correction:** Opus 4-7 scores 79.4% overall on mini-v4-sample, with two real 0/10 blind spots — `scale_dependent_rendering` (shared with Sonnet 4-6) and `zero_width_space_split`† (0/10 re-confirmed for Opus in the 2026-06-12 PDF re-run — still on the pre-redesign family; the 0.6.1 redesign has not been measured yet, and Gemini Flash Lite's 0% likewise awaits a re-run).
