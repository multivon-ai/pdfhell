...
- **The redesigned families have NOT yet been measured.** The 2026-06-12 PDF re-run ([`published_runs/2026-06-12-cross-modality/`](../../published_runs/2026-06-12-cross-modality/)) used the pre-redesign (≤0.6.0) suite build — it re-confirms Opus 4-7 at 0/10 on the *retired* `zero_width_space_split` implementation (and 90% on the retired `unicode_confusable_total`), nineteen days after the original run. No model has been run against the 0.6.1 redesigns yet; until that re-run lands, every published number for these two families measures the retired implementations. Tracked in [#8](https://github.com/multivon-ai/pdfhell/issues/8).
